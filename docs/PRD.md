...
- **Deterministic inbound screening before materialization and LLM scoring**: bulk email candidates must first match the strict software-and-AI title policy. Accepted families are software development (including frontend/backend/full-stack/mobile/product/embedded/test automation), AI/ML/LLM/agentic engineering, platform/infrastructure/DevOps/SRE/cloud, and data engineering. Data science/analytics, developer relations, management, sales/solutions/support engineering, AI training/annotation, and non-software engineering are rejected before a `JobPost`, company, route, or scoring job is created; aggregate screening counts/reasons remain on the inbound email. Accepted candidates are then location-triaged before OpenRouter: Vancouver is highest priority, then Calgary, then remote, with unknown or broad Canada/BC/Alberta allowed at lower priority. Manual imports bypass automatic title screening because they are deliberate owner choices.
...
- **Application assistance**: a job-detail **Cover letter** section explicitly generates, displays, copies, and regenerates one current tailored letter from the posting and synced resume. It is independent of an `Application`: it never changes tracker state, creates an autofill payload, or submits anything. The existing application-draft path separately provides tailored resume emphasis, cover letter/message drafts, structured answers, and reviewable/editable autofill payloads for known form systems.
...
- **Intake management** (INTAKE): inbound volume exceeds what one owner can apply to, so the Jobs
  ...
  pages also let the owner shed and organize intake. Strict pre-materialization title screening keeps
  off-scope bulk-alert candidates out of the job store entirely. Each accepted job carries a `lifecycle_state` — **active**
...
- **Installable PWA shell**: manifest, app icon, full-screen standalone display, and an offline-tolerant app shell (custom service worker built by `vite-plugin-pwa`), plus the notification permission / install-guide flow.
...
- The owner can explicitly generate and manually copy a tailored cover letter from a job detail without starting or submitting an application.
