...
- **web/** — `src/components/tracker/tracker.test.tsx` (`FE-22`/`UI-04`/`UI-14`/`UI-15`/`UI-16`/`UI-17`/`UI-18`/`UI-19`/`UI-20`/`UI-21`):
  ...
  current-status/stage and dismissal no-ops, calendar date selection and Clear, the note editor's
  explicit Save/Cancel and inline validation error, blank `No stage` writes, note and follow-up
