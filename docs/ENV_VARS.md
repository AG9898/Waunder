...
| `OPENROUTER_MODEL` | No | `google/gemini-2.5-flash-lite` | Model id used for **all** LLM calls (scoring, summaries, drafts) — single model, no per-task tiers (RESOLVED-17). Low-cost default (the former `google/gemma-4-31b-it:free` free tier became persistently 429 rate-limited); configurable to any OpenRouter model id. | `api` runtime |
