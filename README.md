...
| Service | Stack |
|---------|-------|
| `web/`     | Vite + React + TypeScript (TanStack Query, React Router, zod), Vitest; built to static assets served by Caddy, which also proxies `/api/*` plus the Resend inbound webhook path to Rails |
...
**web/** — Vite + React + TypeScript PWA (npm scripts in `web/package.json`):

```bash
cd web
npm ci
npm run dev        # Vite dev server on :8000, proxying /api to API_INTERNAL_URL (default :3000)
npm test           # vitest run
npm run build      # vite build (production bundle + service worker)
```
...
Waunder deploys to **Railway** as three containerized services in one project — `web`, `api`, and `worker` — plus managed PostgreSQL. Services communicate over Railway's private network. The browser only ever talks to the `web` origin: Caddy proxies requests under `/api/*` and `/webhooks/resend/inbound` server-side to the Rails `api` service via `API_INTERNAL_URL`. This keeps the frontend same-origin (no CORS), keeps the service worker scope and push registration clean, and means the Rails API needs no public domain.
