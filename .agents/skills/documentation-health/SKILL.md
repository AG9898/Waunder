...
Invoke as `$documentation-health` for a full sweep, optionally specifying whether the run is audit-only or includes cleanup.

## Scope and Modes

Read `AGENTS.md` first. It defines the repository's local documentation routes and maintenance rules.
...
2. Discover the routing topology from `AGENTS.md`, primary index candidates (`docs/INDEX.md` and `docs/README.md`), and nested section indexes or READMEs.
...
An existing major document is a core/platform document listed by the primary routing map or directly routed from `AGENTS.md`. Count each existing major document affected by a structural operation once; do not evade the threshold by fragmenting the plan. A new focused document alone does not make the work major.
