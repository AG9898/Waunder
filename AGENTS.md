...
- Never retry a write to Rails automatically; a replayed write can re-dispatch a trusted submit.
...
### 2026-09-10 — Endpoint port: Go's query encoder differs from URLSearchParams in five characters
FE-05 ported all 25 `RailsClient` methods into `client/src/api/endpoints.ts`, and the only real
trap was `JobFeedParams.query().Encode()`. `URLSearchParams.toString()` is NOT a drop-in
replacement: Go's `url.Values.Encode` **sorts keys** (URLSearchParams preserves insertion order)
and its `QueryEscape` percent-encodes `!*'()` while leaving `~` literal — `encodeURIComponent` does
the exact opposite on all five. `jobFeedQuery` therefore builds pairs by hand, sorts them byte-wise,
and post-processes `encodeURIComponent` output (escape `!*'()`, then `%20` → `+`). Two Go semantics
that look like bugs but are intentional: a filter is omitted when `TrimSpace(val) == ""` yet the
value sent is the **untrimmed** original, and `page` is sent only above 1. The expectations in
`endpoints.test.ts` were generated by actually running the Go method in a throwaway `go run`
program and pasting its output, which is the only way that block is evidence rather than a mirror
of the TypeScript — do the same if the encoder is ever touched. Also: TanStack Query's default
`retry: 3` is wrong for this app in two directions — it would retry a 401 three times before the
